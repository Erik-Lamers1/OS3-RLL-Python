...
class Challenge:
    ...
    def get_challenge(args):
        ...
        # TODO: Check that the timeout of the challenge is not exceeded
        :type args: Player ID
        """
        # Get the player ID
        pid = Player.get_id_by_gamertag(args[0])

        db.execute('SELECT date,p1,p2 FROM challenges WHERE (p1="{}" OR p2 ="{}") AND winner IS NULL'.format(pid, pid))
        res = db.fetchone()
